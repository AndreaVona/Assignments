...
static int cmd_pub(int argc, char **argv) //I changed this function in order to send all data values
{
    ...
    {
	char dataToSend[200]; //An array in which I can store the values
        unsigned long long int ts = ((unsigned long long)time(NULL)) * 1000; //Timestamp
	
	if(firstIteration==0) {
		sprintf(dataToSend, "{ \"device\":\"Station-2\"}"); //{"device":"Device A"}
	} else {
		...
		// this are the data we are going to send:
        	sprintf(dataToSend, "{ \"Station-2\": [ {\"ts\": %llu, \"values\":{\"temperature\": \"%.2f\", \"humidity\": \"%.2f\", \"windDirection\": \"%.2f\", \"windIntensity\": \"%.2f\", \"rainHeight\": \"%.2f\"}}]}", ts, temp, hum, winDir, winInt, rain);
	...
	if(firstIteration==0) {
		printf("\nPub on this topic v1/gateway/connect this values:\n%s\n", dataToSend);
	} else {
	...
	if (emcute_reg(&t) != EMCUTE_OK) {
        ...
        /* step 2: publish data */
        if (emcute_pub(&t, dataToSend, strlen(dataToSend), flags) != EMCUTE_OK) {
