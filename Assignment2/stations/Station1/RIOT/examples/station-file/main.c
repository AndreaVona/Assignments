...
static int cmd_pub(int argc, char **argv) //I changed this function in order to send all data values
{
	...
	if(firstIteration==0) {
		sprintf(dataToSend, "{ \"device\":\"Station-1\"}"); //{"device":"Device A"}
	} else {
		...
		// this are the data we are going to send:
        	sprintf(dataToSend, "{ \"Station-1\": [ {\"ts\": %llu, \"values\":{\"temperature\": \"%.2f\", \"humidity\": \"%.2f\", \"windDirection\": \"%.2f\", \"windIntensity\": \"%.2f\", \"rainHeight\": \"%.2f\"}}]}", ts, temp, hum, winDir, winInt, rain);
